...
$exposition = $_POST["EXPOSITON"];
$exposition = stripslashes ($exposition);
...
if(isset($_POST["displayTop"]))
{
 	$displayTop= $_POST["displayTop"];
	   $displayTop = floatval($displayTop);
              if ($displayTop >=11)
	         echo 'The number is too big or invalid. The maximum number is 10. Please go back and try again';
}

$webOnly = 'no';
if(isset($_POST["VOTINGTYPE"]))
{
	if($_POST["VOTINGTYPE"]=='web')
		$webOnly = 'yes';
}
...
if(!$webOnly)
{
	...
	{
		echo 'The duration you entered is too big or invalid. A Telephone&Web voting can run up to 8 hours. Please go back and try again';
		ob_end_flush();
		exit;
	}
}
...
if(odbc_fetch_row($queryexe))
{
	...
	echo 'A survey with the same title already exists, please <a href="/index.php?title=Create_Rank_Expositions">go back</a> and choose another title for your survey or watch the existing survey <a href="/index.php?title='.$encodedTitle.'">'.$title.'</a>.';
...
$hduration=$duration/60;
//main wiki text area
$wikiText.="<RankExposition multipleVoting=$allowInvalidVotes anonymous=$allowAnonymousVotes duration=$hduration SMSreply=$isSMSRequired webonly=$webOnly displayTop=$displayTop>\n";
$wikiText.="you can edit and write your question below without removing the # mark \n";
$wikiText.="# The Question  \n";

if(isset($_POST["CHOICES"]))
{
      $ChoiContent=explode("\n",$choices);
      $countChoi=substr_count($choices,"\n");
      $i=0;
      while ($i<=$countChoi)
      {
       $wikiText.="##$ChoiContent[$i]\n";
       $i++;
       }
}
$wikiText.="\n";
if(isset($_POST["EXPOSITON"]))
{
      $presContent=explode("\n",$exposition);
      $countContent=substr_count($exposition,"\n");
      $p=0;
      while ($p<=$countContent)
      {
       $wikiText.="*$presContent[$p]\n";
       $p++;
       }
}
$wikiText.="\n</RankExposition>\n";
$wikiText.="Created by $author\n[[Category:Surveys]]\n[[Category:Surveys by $author]]\n[[Category:Surveys in $chcategory]]\n[[Category:Rank Expositions]]";
$req->addPostData('wpTextbox1', $wikiText);

//page summary
$req->addPostData('wpSummary', "New Rank Exposition survey created by $author");
