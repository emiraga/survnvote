...
if($_POST["Submit"]=="Reset survey")
{
	...
	$Query = "DELETE FROM quizresultsms WHERE pageID LIKE '%|$pageID|%'";
...
if ($_POST["Submit"]=="Start survey")
{
	...
	{
		header("Location: ttp://$site_location/index.php?title=$title&action=purge");
		ob_end_flush();
		exit;
	}
	
	$subtractWrong=0;
	if($_POST['subtractwrong']=='yes')
		$subtractWrong=1;
	...
	foreach($_POST["question"] as $question)
	{
		...
		$questionIndex++;
		$comment='';
		if(isset($_POST["q$questionIndex".'comment']))
			$comment=$_POST["q$questionIndex".'comment'];
		$comment=stripslashes($comment);
		$survey->setQuestion($question.$comment);
		$point=1;
		if(isset($_POST["q$questionIndex".'point']))
			$point=$_POST["q$questionIndex".'point'];
		$answer=0;
		if(isset($_POST["q$questionIndex".'answer']))
			$answer=$_POST["q$questionIndex".'answer'];
		$choices = array();
		$choiceIndex=0;
...
else if ($_POST["Submit"]=="Restart survey" || $_POST["Submit"]=="Reset survey")
{
