...
if ($pos !== false) {
   echo 'The quiz title can not have "&" in it, please go back and try again.';
   ob_end_flush();
   exit;
}
$invalidChar  = '_';
$pos = strpos($title, $invalidChar);
if ($pos !== false) {
   echo 'The quiz title can not have "_" in it, please go back and try again.';
   ob_end_flush();
   exit;
}
...
$substractWrong = 'no';
if(isset($_POST["substractwrong"]))
{
	if($_POST["substractwrong"]=='yes')
		$substractWrong = 'yes';
}
...
if(odbc_fetch_row($queryexe))
{
	...
	echo 'A survey with the same title already exists, please <a href="index.php?title=Create_Quiz">go back</a> and choose another title for your quiz or watch the existing survey <a href="/index.php?title='.$encodedTitle.'">'.$title.'</a>.';
...
//main wiki text area
$wikiText="<!--Please use # to define the choices and (x) to mark the right answer-->\n";
$wikiText.="<Quiz SubstractWrong=$substractWrong anonymous=$allowAnonymousVotes duration=$duration SMSreply=$isSMSRequired telephoneVoting=$telephoneVoting webVoting=$webVoting>\n";
$wikiText.="Question1 (2)\n#choice11\n#choice12  (x)\n//comments\n#choice13\nQuestion2 (5)\n#choice21  (x)\n#choice22\n//comments\n";
...
//page summary
$req->addPostData('wpSummary', "New presentation survey created by $author");
