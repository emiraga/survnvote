...
define('SURVNVOTE_DAEMON',true);
define('MEDIAWIKI',true);
@require_once("$IP/LocalSettings.php");
@include_once("$IP/AdminSettings.php");

$vgDBUserName = $wgDBadminuser; //Set this to database user that has priviledges to access
...
require_once("$vgPath/special/SurvnvoteStats.php");
...
function vfVoteFromDaemon($choice, UserVO &$user)
{
    ...
    if($status == 'active' && $userperm->canVote($page, 'phone'))
    {
        ...
        $votedao->vote($votevo);
        //update statistics
        SurvnvoteStatsUpdate::addSmsVote();
    }
}
...
if(!isset($args[1]) || $args[1] == 'debug' || $args[1] == 'daemon')
{
    ...
    while(1)
    {
        ...
        {
            error_log('Survnvote daemon error: '.$e->getMessage() . ' ' .$e->getFile().' '.$e->getLine());
