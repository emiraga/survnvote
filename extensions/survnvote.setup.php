...
{
    die('$wgSecretKey is not specified or it is too short, survnvote requires this value to be '
            .'set at least 20 characters. http://www.mediawiki.org/wiki/Manual:$wgSecretKey');
}
if(! isset($wgScriptPath) || strlen($wgScriptPath) < 1 )
{
    die('$wgScriptPath is not specified or it is too short, survnvote requires this value to be '
            .'set. http://www.mediawiki.org/wiki/Manual:$wgScriptPath');
}
if(! isset($IP) || strlen($IP) < 1 )
{
    die('$IP is not specified or it is too short, survnvote requires this value to be '
            .'set. http://www.mediawiki.org/wiki/Manual:$IP');
}
if(! isset($wgDBTableOptions) )
{
    die('$wgDBTableOptions is not specified, survnvote requires this value to be '
            .'set. http://www.mediawiki.org/wiki/Manual:$wgDBTableOptions');
}

if(!defined('SURVNVOTE_TEST'))
    echo '<html><head><title>Survnvote installation</title></head><body>';

global $vgScript, $wgScriptPath;
if(defined('SURVNVOTE_TEST') || isset($_POST['do_install']))
{
    try
    {
        vfDoSetup();
        if(!defined('SURVNVOTE_TEST'))
            echo "<h1>Survnvote installation is complete.</h1>\n";
    }
    catch(Exception $e)
    {
    ...
    {
        echo "When you are done with installation, <u>please</u> delete the file <b>survnvote.setup.php</b> from extensions directory.<br>\n";
...
else
{
    ...
    echo <<<END_HTML
<h1>Welcome to survnvote installation.</h1>
<p>This script <b>survnvote.setup.php</b> is very dangerous and must be deleted after installation is complete.</p>
<p>Do not run this script if you have already installed survnvote, it will <b>truncate</b> tables from database.</p>
Installation Steps:
<ol>
<li>Make sure that MediaWiki is working properly.</li>
<li>Rename <code>extensions/survnvote/config.sample</code> to <code>config.php</code>, and configure survnvote settings.</li>
<li>Add following line to <a href="http://www.mediawiki.org/wiki/Manual:LocalSettings.php"><b>LocalSettings.php</b></a><br />
    <code><br>require_once("\$IP/extensions/survnvote/survnvote.php");<br><br></code></li>
...
<li>
<form action="$vgScript/../survnvote.setup.php" method="POST">
 ...
 * function vfDoSetup:
 * 
 * Perform actions of survnvote setup
 *
